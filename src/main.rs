...
async fn main() -> Result<(), ShipItError> {
    ...
    match args.command {
        cli::Commands::B2b { source, target, dir, id, platform, remote, prompt, description, .. } => {
            commands::b2b::branch_to_branch(&ctx, source, target, dir, id, platform, remote, prompt, description).await?;
        }
