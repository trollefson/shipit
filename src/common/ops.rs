...
use crate::common::{
    create_github_release, create_github_release_for_tag, create_gitlab_release, create_gitlab_tag,
    extract_repo_path, fetch_github_pr_info, fetch_gitlab_mr_info, fetch_latest_github_tag,
    fetch_latest_gitlab_tag, lookup_github_identifier, lookup_gitlab_project_id, next_version,
    open_merge_request, parse_github_pr_number, parse_gitlab_mr_iid, Github, Gitlab, OllamaAgent,
    ShipitAgent,
};
...
/// Fetches the name of the most recent tag from the remote platform.
pub(crate) async fn fetch_latest_platform_tag(
    ctx: &Context,
    resolved_id: &str,
    is_github: bool,
    is_gitlab: bool,
    ...
    if is_github {
        let token = ctx
            ...
            .token
            .as_deref()
            .ok_or_else(|| ShipItError::Error("GitHub token is required to fetch the latest tag.".to_string()))?;
        let parts: Vec<&str> = resolved_id.splitn(2, '/').collect();
        if parts.len() != 2 {
            return Err(ShipItError::Error(format!(
                "GitHub project identifier '{}' must be in 'owner/repo' format.",
        ...
        fetch_latest_github_tag(&ctx.settings.github.domain, token, parts[0], parts[1]).await
    } else if is_gitlab {
        let token = ctx
            .settings
            .gitlab
            .token
            .as_deref()
            .ok_or_else(|| ShipItError::Error("GitLab token is required to fetch the latest tag.".to_string()))?;
        let project_id: u64 = resolved_id.parse().map_err(|_| {
            ShipItError::Error(format!(
                "GitLab project identifier '{}' must be a numeric project id.",
                resolved_id
        ...
        fetch_latest_gitlab_tag(&ctx.settings.gitlab.domain, token, project_id).await
    } else {
        Err(ShipItError::Error(
            "Could not determine platform from remote url. Ensure the remote url contains 'github' or 'gitlab'.".to_string(),
        ))
...
/// Creates a release on the remote platform for an already-existing tag.
pub(crate) async fn create_platform_release(
    ctx: &Context,
    resolved_id: &str,
    tag_name: &str,
    notes: &str,
    ...
    if is_github {
        ...
        let token = ctx
            ...
            .token
            .as_deref()
            .ok_or_else(|| ShipItError::Error("GitLab token is required to create a release.".to_string()))?;
        ...
        .await
        .map_err(|e| ShipItError::Error(format!("Failed to create a GitLab release: {}", e)))
    } else {
        Err(ShipItError::Error(
            "Could not determine platform from remote url. Ensure the remote url contains 'github' or 'gitlab'.".to_string(),
        ))
