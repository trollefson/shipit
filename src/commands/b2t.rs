...
pub async fn branch_to_tag(ctx: &Context, args: B2tArgs) -> Result<(), ShipItError> {
    ...
    let mut summary = if let Some(provided) = args.description {
        ...
        println!("Comparing against tag: {}", latest_tag_name);
        ...
        if messages.is_empty() {
            println!("No commits found since tag '{}'. Nothing to do.", latest_tag_name);
            return Ok(());
        }
    ...
    .await?;
    println!("The tag is available at:\n\n{}", url);
